...
All requests should include accept header, to specify the format of data to be returned.
...
All responses will include a HTTP status code in the response. Some of the most common status codes that each type of request may return, along with it's definitions, are listed below.
...
| name <br> *required*        | Name of phonebook entry  <br> *String*                                 |  
| phoneNumber <br> *required* | Phone number of phonebook entry  <br> *String*                         |  
| speedDial <br> *optional*   | Up to four digit speed-dial number for phonebook entry  <br> *Integer* |  
...
| uri  <br> *read-only*       | Unique URI for each resource <br> *String*                            |  
| created <br> *read-only*    | Time that the resource was created in ISO8601 format <br> *String*    |  
| direction <br> *read-only*  | Direction of message (`in`/`out`) <br> *String*                       |  
...
| sendStatus <br> *read-only* | Indicates message send status `pending`/`sent`/`failed` <br> *String* |  
...
| cost <br> *read-only*       | Message cost in GBP (£) <br> *String*                                 |
...
| callType <br> *required*    | Whether the call was internal or not (`domain`/`OUT`). <br> *String*                                                                                            |  
| direction <br> *read-only*   | Direction of call (`IN`/`OUT`). <br> *String*                                                                                                                   |  
...
| endpoint <br> *required*     | The name or phone number of recipient. <br> *String*                                                                                                            |  
| callStarted <br> *read-only* | The time that the call began in ISO8601 format. <br> *String*                                                                                                   |  
| outcome <br> *read-only*     | Whether or not the call was answered (`answered`/`no-answer`). <br> *String*                                                                                    |  
| duration <br> *read-only*    | The duration of call in seconds. <br> *Integer*                                                                                                                  |  
| cost <br> *read-only*        | The cost of call in GBP (£). <br> *Integer*                                                                                                                      |  
...
| callId <br> *read-only*    | A unique ID for each leg of the call. <br> *String*                                                                                                             |  
| linkedId <br> *read-only*  | The unique ID of the call itself.  This ID is the same across all legs of a single call and can be used to link multiple legs of a call together. <br> *String* |  
| endpoint <br> *read-only*  | URI to the endpoint of the [???] <br> *String*                                                                                                                  |  
...
| repeat <br> *required*   | When the time interval repeats. <br> *String: WEEKLY, YEARLY*                |  
...
| shortNumber <br> *read-only*       | 3 digit short number identifier for this extension. <br> *String*             |  
| name <br> *read-only*              | Name assigned to the extension. <br> *String*                                 |  
...
| callRecording <br> *read-only*     | Whether call recordings are enabled `ALWAYS`/`OFF`/`ON_DEMAND`. <br> *String* |  
...
| links[].*xyz* <br> *read-only*       | Link to associated resource *xyz*. <br> *Array*                                 |  
...
| destination <br> *required*                 | The destination that this number will route calls to. <br> *String*                        |  
| smsEnabled <br> *required*                  | Whether SMS messages can be sent from this number. <br> *String*                           |  
| smsAllowIncoming <br> *required*            | Whether SMS messages can be sent to this number. <br> *String*                             |  
| smsNotificationEmail <br> *required*        | The email address to send incoming SMS notifications to. <br> *String*                     |  
| faxEnabled <br> *required*                  | Whether this number can receive faxes. <br> *String*                                       |  
| routingRules[] <br> *required*              | Routing rules that apply to this number. <br> *String*                                     |  
...
| allowCalls <br> *required* | Whether calls can be made to this number. <br> *String*                           |  
| allowSms <br> *required*   | Whether sms messages can be sent to this number. <br> *String*                    |  
...
| accountType <br> *required*     | The type of account (`prepaid`/`postpaid`). <br> *String*              |  
| creditRemaining <br> *read-only* | Name or number of outgoing caller id. <br> *String*                    |  
...
| inclusiveMins <br> *required* | Total number of minutes in the bundle. <br> *String*                  |  
| priceMonthly <br> *required*  | Monthly cost in GBP (£). <br> *String*                                |  
...
| usedMins <br> *required*      | Bundle minutes remaining. <br> *String*                               |  
| availableMins <br> *required* | Bundle minutes used. <br> *String*                                    |  
| autoRenew <br> *required*     | Whether the bundle set to automatically renew. <br> *String*          |  
| status <br> *required*        | Returns either "ACTIVE" or "EXPIRED". <br> *String*                   |  
